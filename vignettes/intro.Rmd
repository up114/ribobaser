...
We enable two ways to calculate TE. We also have the option to run principal component removal on the Ribo and RNA matrices, either giving a specific number of PCs to remove or using "auto", which calculate the number of PCs to remove using sva::num.sv(..., method = "be").
...
In addition to the option of removing principal components on the Ribo and RNA matrices, principal components can also be removed from the TE matrix itself. PC removal can be run on a global or study-level scale. In this example, all 3 of the experiments are from the same study and therefore, the global and study outputs match.
